...
function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-950 via-slate-950/95 to-slate-900 text-slate-100">
      <nav
        className="sticky top-0 z-50 border-b border-white/5 bg-slate-950/80 backdrop-blur-xl supports-backdrop-filter:bg-slate-950/70"
        aria-label="Administrative function navigation"
      >
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-6 px-6 py-4">
          <div className="flex items-center gap-2">
            <span className="text-lg font-semibold tracking-tight text-slate-100">AutoNode</span>
          ...
          <div className="flex items-center gap-1">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="text-slate-200 hover:bg-white/10 hover:text-slate-100">
                  Users & Access
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" className="w-56">
                <DropdownMenuLabel>Access Control</DropdownMenuLabel>
                <DropdownMenuItem>Create user account</DropdownMenuItem>
        ...
        <div className="relative mx-auto max-w-6xl px-6 py-14">
          <div className="flex flex-wrap items-start justify-between gap-10">
            <div className="max-w-2xl space-y-5">
              <Badge className="bg-slate-100/10 text-slate-100">Admin command center</Badge>
              <h1 className="text-4xl font-semibold tracking-tight sm:text-5xl">Operational overview</h1>
              ...
              <div className="flex flex-wrap gap-4 text-sm text-slate-200/70">
                <span>Service uptime (7d): <strong className="font-semibold text-slate-100">99.87%</strong></span>
                <span>Mean response: <strong className="font-semibold text-slate-100">212 ms</strong></span>
                <span>Last sync: <strong className="font-semibold text-slate-100">08:24 UTC</strong></span>
              </div>
            </div>

            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <Button variant="outline" size="sm" className="backdrop-blur border-white/20 bg-white/5 text-slate-100">
                Schedule maintenance window
              </Button>
              <Button size="sm" className="shadow-[0_12px_40px_-20px_rgba(56,189,248,0.65)]">
                Broadcast status update
              </Button>
            </div>
          </div>

          <Separator className="my-10 border-white/10 bg-white/10" />

          <div className="-mx-2 flex snap-x gap-4 overflow-x-auto pb-4">
            {serviceClusters.map((service) => (
              <Card
                key={service.name}
                className="min-w-65 flex-1 snap-start border-white/10 bg-white/5 backdrop-blur lg:min-w-0"
              >
                <CardHeader className="space-y-2">
                  ...
                  <CardTitle className="text-lg font-semibold text-slate-100">{service.name}</CardTitle>
                  <CardDescription className="text-slate-200/80">{service.summary}</CardDescription>
                </CardHeader>
                <CardFooter className="flex items-center gap-2 text-sm text-slate-200/80">
      ...
      <main className="mx-auto max-w-6xl space-y-12 px-6 py-12">
        <section>
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div className="space-y-2">
              <h2 className="text-2xl font-semibold tracking-tight">Spotlight metrics</h2>
              <p className="text-sm text-slate-200/70">Signals that moved the most in the past day across your network.</p>
            </div>
            <Button variant="ghost" size="sm" className="text-slate-200/80">
              Configure alerts
            </Button>
          </div>
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {spotlightMetrics.map((metric) => (
              <Card key={metric.label} className="border-white/10 bg-white/5 backdrop-blur">
                <CardHeader className="space-y-1">
                  <CardDescription className="text-slate-200/70">{metric.label}</CardDescription>
                  <CardTitle className="text-3xl font-semibold text-slate-100">{metric.value}</CardTitle>
                </CardHeader>
                <CardContent className="text-sm text-slate-200/70">
                  <span className={`font-semibold ${metric.tone}`}>{metric.delta}</span>
                  {metric.delta ? <span className="mx-2 text-slate-500/50">|</span> : null}
        ...
        <section className="grid gap-6 lg:grid-cols-[2fr_1.3fr]">
          <Card className="border-white/10 bg-white/5 backdrop-blur">
            <CardHeader className="space-y-1">
              <CardTitle className="text-xl">Strategic initiatives</CardTitle>
            ...
            <CardContent className="space-y-6">
              {initiatives.map((initiative) => (
                <div
                  ...
                  className="rounded-3xl border border-white/10 bg-white/5 px-5 py-4 text-sm text-slate-200/80"
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <h3 className="text-base font-semibold text-slate-100">{initiative.title}</h3>
          ...
          <div className="space-y-6">
              ...
              <CardContent className="space-y-4 text-sm text-slate-200/80">
                <div className="rounded-3xl border border-white/10 bg-white/5 px-4 py-3">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-slate-100">DOT quarterly audit</span>
                    <Badge variant="outline" className="border-amber-300/40 bg-amber-200/10 text-amber-200">
                      Due in 2 days
                    </Badge>
                  </div>
                  <p className="mt-2 text-slate-200/70">Collect 12 outstanding maintenance certificates for heavy duty class.</p>
                </div>

                <div className="rounded-3xl border border-white/10 bg-white/5 px-4 py-3">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-slate-100">Driver compliance</span>
                    <Badge variant="outline" className="border-emerald-300/40 bg-emerald-200/10 text-emerald-200">
                      92% complete
                    </Badge>
                  </div>
                  <p className="mt-2 text-slate-200/70">14 drivers pending fatigue management acknowledgment.</p>
                </div>

                <div className="rounded-3xl border border-white/10 bg-white/5 px-4 py-3">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-slate-100">Security posture</span>
                    <Badge variant="outline" className="border-sky-300/40 bg-sky-200/10 text-sky-200">
                      Stable
                    </Badge>
                  </div>
                  <p className="mt-2 text-slate-200/70">No elevated permission requests in the last 6 hours.</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-white/10 bg-white/5 backdrop-blur">
              <CardHeader className="space-y-1">
                ...
                <CardDescription className="text-slate-200/70">Latest signals that need your attention today.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-5 text-sm text-slate-200/80">
